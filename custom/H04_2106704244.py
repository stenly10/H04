...
class MyTopo( Topo ):
    def build( self ):
        ...
        switch_k = self.addSwitch('Switch K')
        switch_i = self.addSwitch('Switch I')
        switch_s = self.addSwitch('Switch S')
        switch_r = self.addSwitch('Switch R')
        ...
        for switch in ['Switch K', 'Switch I', 'Switch S', 'Switch R']:
            if switch == 'Switch K':
                ...
                for j in range(number_host_per_switch_2):
                    ...
                    self.addLink(host_name, switch_r)
